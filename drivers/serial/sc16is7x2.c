...
static void sc16is7x2_handle_rx(struct sc16is7x2_chip *ts, unsigned ch)
{
	...
	struct tty_struct *tty = uart->info->port.tty;  //uart->state->port.tty;
...
static void sc16is7x2_handle_tx(struct sc16is7x2_chip *ts, unsigned ch)
{
	struct sc16is7x2_channel *chan = &ts->channel[ch];
	struct uart_port *uart = &chan->uart;
	struct circ_buf *xmit = &uart->info->xmit;  // state->xmit;
	unsigned long flags;
	unsigned i, len;
	int txlvl;
		       ...
		       struct ktermios *old)
{
	...
	printk(KERN_INFO "ttysac%d baud=%u quot=%u.\n",port->line,baud,chan->quot);
		...
		struct sc16is7x2_platform_data *pdata)
{
	...
	ts->gpio.names = (char**)pdata->names;
...
static int __devinit sc16is7x2_probe(struct spi_device *spi)
{
	...
	if (ret)
		goto exit_destroy;
	//printk(KERN_INFO "sc16is7x2 register ttySAC%d!\n",pdata->uart_base);
	
	ret = sc16is7x2_register_uart_port(ts, pdata, 1);
	if (ret)
		goto exit_uart0;
        //printk(KERN_INFO "sc16is7x2 register ttySAC%d!",pdata->uart_base+1);
		
	ret = sc16is7x2_register_gpio(ts, pdata);	
	if (ret)
		goto exit_uart1;
        //printk(KERN_INFO "sc16is7x2 register GPIO successful!");
	/*
	dev_info(&spi->dev, DRIVER_NAME " at CS%d (irq %d), 2 UARTs, 8 GPIOs\n"
			"    ttySAC%d, ttySAC%d, gpiochip%d\n",
			spi->chip_select, spi->irq,
			pdata->uart_base, pdata->uart_base + 1,
			pdata->gpio_base);  */
	dev_info(&spi->dev, DRIVER_NAME " at CS%d (irq %d), 2 UARTs\n"
			"    ttySAC%d, ttySAC%d\n",
			spi->chip_select, spi->irq,
			pdata->uart_base, pdata->uart_base + 1);		
...
MODULE_ALIAS("spi platform:"DRIVER_NAME);
