...
#include <mach/gpio.h>	// add by dongking at 2011-07-01, like as timll
#include <asm/io.h>
#include <asm/sizes.h>
#include <asm/gpio.h>
...
{
	return !at91_get_gpio_value(cf->board->det_pin);	// modify by dongking at 2011-07-01, like as timll
}
...
static irqreturn_t at91_cf_irq(int irq, void *_cf)
{
	...
	if (irq == cf->board->det_pin) {
		...
		if (present != cf->present) {
			cf->present = present;
//			pr_debug	// modify by dongking at 2011-07-01, like as timll
			printk("%s: card %s\n", driver_name,
...
static int at91_cf_get_status(struct pcmcia_socket *s, u_int *sp)
{
	...
	if (at91_cf_present(cf)) {
		...
		*sp = SS_DETECT | SS_3VCARD;
		if (!rdy || at91_get_gpio_value(rdy))	// modify by dongking at 2011-07-01, like as timll
			*sp |= SS_READY;
		if (!vcc || at91_get_gpio_value(vcc))	// modify by dongking at 2011-07-01, like as timll
			*sp |= SS_POWERON;
	} else
		*sp = 0;
...
at91_cf_set_socket(struct pcmcia_socket *sock, struct socket_state_t *s)
{
	...
	if (cf->board->vcc_pin) {
		switch (s->Vcc) {
			case 0:
				at91_set_gpio_value(cf->board->vcc_pin, 0);	// modify by dongking at 2011-07-01, like as timll
				break;
			case 33:
				at91_set_gpio_value(cf->board->vcc_pin, 1);	// modify by dongking at 2011-07-01, like as timll
				break;
			default:
				return -EINVAL;
		}
	...
	at91_set_gpio_value(cf->board->rst_pin, s->flags & SS_RESET);	// modify by dongking at 2011-07-01, like as timll
